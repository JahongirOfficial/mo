...
export function KirishSahifasi() {
  ...
  // Telefon raqamni formatlash: XX XXX XX XX (faqat raqamlar)
  ...
  // Submit uchun to'liq telefon raqam
  const getFullPhone = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (activeTab === 'login') {
        ...
        await register(fullName, getFullPhone(), password);
      }
      navigate('/bolim');
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-50 font-display flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-16">
        <div className="w-full max-w-md">
          ...
          {/* Tabs */}
          <div className="flex bg-slate-100 rounded-xl sm:rounded-2xl p-1 sm:p-1.5 mb-6 sm:mb-8">
            <button
              onClick={() => { setActiveTab('login'); setError(''); }}
              className={`flex-1 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-semibold transition-all text-sm sm:text-base ${
                activeTab === 'login' 
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {activeTab === 'register' && (
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">To'liq ismingiz</label>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-24 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-xl focus:bg-white focus:border-primary outline-none transition-all text-slate-900 font-medium"
            ...
            {activeTab === 'login' && (
              <div className="flex justify-end">
                <a href="#" className="text-sm font-semibold text-primary hover:text-emerald-700 transition-colors">
            ...
            >
              {loading ? (
                ...
                <>
                  {activeTab === 'login' ? 'Kirish' : 'Hisob yaratish'}
